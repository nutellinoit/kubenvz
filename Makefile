...
test-kustomize:
	...
	dist/kubenvz kustomize list remote | sort | xargs -n 1 -P 2 dist/kubenvz kustomize install

test-kubectl:
	rm -rf ~/.kubenvz/kubectl*
	dist/kubenvz kubectl list remote | sort | xargs -n 1 -P 2 dist/kubenvz kubectl install
